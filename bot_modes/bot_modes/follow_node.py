...
class HumanFollowerLifecycle(Node):
    ...
    def __init__(self,node_name, **kwargs):
        ...
        self.max_speed = 1.0      
        self.max_turn = 0.5
        self.get_logger().info("Human Follower Node started, currently Unconfigured")
        

    def on_configure(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Human Follower configured...")
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        ...
        self.get_logger().info("Human Follower activated...")
    ...
    def on_deactivate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Human Follower deactivated...")
        return super().on_deactivate(state)

    def on_cleanup(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Human Follower cleaned up...")
        if self.detection_subscription:
            self.destroy_subscription(self.detection_subscription)
        return TransitionCallbackReturn.SUCCESS

    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Shutting Down Human Follower")
        if self.detection_subscription:
            self.destroy_subscription(self.detection_subscription)
        return TransitionCallbackReturn.SUCCESS
...
def main(): 
    ...
    follow_node = HumanFollowerLifecycle('follow_node')
